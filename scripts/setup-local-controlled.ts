...
function localEnvironment(): string[] {
  return [
    ...
    `EXPECTED_NETWORK=${LOCAL_CONTROLLED.networkId}`,
    `EXPECTED_ECOSYSTEM_ID=${LOCAL_CONTROLLED.ecosystemId}`,
    `EXPECTED_CREDENTIAL_SCHEMA_ID=${LOCAL_CONTROLLED.credentialSchemaId}`,
    `EXPECTED_ISSUER_DID=${LOCAL_CONTROLLED.issuerDid}`,
    `EXPECTED_VERIFIER_DID=${LOCAL_CONTROLLED.verifierDid}`,
    `VERANA_RESOLVER_URL=http://localhost:${LOCAL_CONTROLLED.ports[2]}`,
